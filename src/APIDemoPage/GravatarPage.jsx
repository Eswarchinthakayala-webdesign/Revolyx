...
/* ---------- Default resource (user provided) ---------- */
...
/* ---------- Helpers: small MD5 implementation (lightweight) ---------- */
/* Minimal MD5 implementation (licensed permissively; inline for offline use).
   Source adapted for brevity and clarity (suitable for client-side hashing). */
...
function md5(s) {
  // Adapted simple implementation that works for ASCII and many unicode cases (not fully rigorous).
  // For production, prefer a well-tested md5 library.
  let i;
...
export default function GravatarPage() {
  ...
  const [defaultImage, setDefaultImage] = useState("identicon"); // fallback types: 404, mp, identicon, monsterid, wavatar, retro, robohash
  const [loading, setLoading] = useState(false);
  const [imageUrl, setImageUrl] = useState(buildGravatarUrl(DEFAULT_RESOURCE.exampleHash, 400, "g", "identicon"));
  ...
  async function fetchAvatar() {
    ...
    // Attempt to fetch Gravatar profile JSON: https://en.gravatar.com/<hash>.json
    try {
      const profileUrl = `https://en.gravatar.com/${hashInput}.json`;
      const res = await fetch(profileUrl, { method: "GET" });
      if (!res.ok) {
        // 404 means no profile; that's fine
        setProfileJson(null);
  ...
  function chooseFavorite(f) {
    setHashInput(f.hash);
    setImageUrl(f.thumb);
    // try fetch profile for chosen favorite
    fetchProfile(f.hash);
  }
  ...
  return (
    <div className={clsx("min-h-screen p-6 max-w-8xl mx-auto")}>
      ...
      <header className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className={clsx("text-3xl md:text-4xl font-extrabold")}>Gravatar — Avatar Explorer</h1>
          <p className="mt-1 text-sm opacity-70">Fetch Gravatar avatars by email or MD5 hash. View profile JSON, save favorites, and debug endpoints.</p>
        </div>
          ...
          <Card className={clsx("rounded-2xl overflow-hidden border", isDark ? "bg-black/40 border-zinc-800" : "bg-white/90 border-zinc-200")}>
            <CardHeader className={clsx("p-5 flex items-center flex-wrap gap-3 justify-between", isDark ? "bg-black/60 border-b border-zinc-800" : "bg-white/90 border-b border-zinc-200")}>
              ...
              <div className="flex items-center gap-2">
                <div className="flex items-center gap-2">
                  ...
                  <select value={size} onChange={(e) => { setSize(Number(e.target.value)); setImageUrl(buildGravatarUrl(hashInput || DEFAULT_RESOURCE.exampleHash, Number(e.target.value), rating, defaultImage)); }} className="rounded-md border px-2 py-1 bg-transparent">
                    {[80, 128, 200, 400, 800].map((s) => <option key={s} value={s}>{s}px</option>)}
                  </select>

                  <label className="text-xs opacity-60">Rating</label>
                  <select value={rating} onChange={(e) => { setRating(e.target.value); setImageUrl(buildGravatarUrl(hashInput || DEFAULT_RESOURCE.exampleHash, size, e.target.value, defaultImage)); }} className="rounded-md border px-2 py-1 bg-transparent">
                    {["g", "pg", "r", "x"].map((r) => <option key={r} value={r}>{r.toUpperCase()}</option>)}
                  </select>

                  <label className="text-xs opacity-60">Default</label>
                  <select value={defaultImage} onChange={(e) => { setDefaultImage(e.target.value); setImageUrl(buildGravatarUrl(hashInput || DEFAULT_RESOURCE.exampleHash, size, rating, e.target.value)); }} className="rounded-md border px-2 py-1 bg-transparent">
                    {["404", "mp", "identicon", "monsterid", "wavatar", "retro", "robohash"].map((d) => <option key={d} value={d}>{d}</option>)}
                  </select>
                </div>

                <Button variant="outline" onClick={() => fetchAvatar()}>{loading ? <Loader2 className="animate-spin" /> : <ImageIcon />} Fetch</Button>
                <Button variant="ghost" onClick={() => setDialogOpen(true)}><List /> View Image</Button>
                <Button variant="outline" onClick={() => saveFavorite()}><Star /> Save</Button>
              </div>
            </CardHeader>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className={clsx("p-4 rounded-xl border flex flex-col items-center gap-3", isDark ? "bg-black/20 border-zinc-800" : "bg-white/70 border-zinc-200")}>
                  <div className="w-full text-sm font-semibold mb-1">Avatar</div>
                  <div className="w-full flex items-center justify-center">
                    <div className="rounded-md overflow-hidden bg-zinc-50 dark:bg-zinc-900 p-2">
                      {loading ? (
                        <div className="w-[200px] h-[200px] flex items-center justify-center"><Loader2 className="animate-spin" /></div>
                      ) : (
                        // eslint-disable-next-line @next/next/no-img-element
                        <img src={imageUrl} alt={`Gravatar ${hashInput}`} className="w-[200px] h-[200px] object-cover rounded-md" />
                      )}
                    </div>
                  </div>
                  ...
                  <div className="w-full space-y-1 text-sm">
                    ...
                    <div className="font-medium break-words text-sm">{hashInput || "—"}</div>
                    ...
                    <div className="flex gap-2 mt-2">
                      <Button variant="outline" onClick={() => copyImageUrl()}><Copy /> Copy URL</Button>
                      <Button variant="outline" onClick={() => downloadImage()}><Download /> Download</Button>
                      <Button variant="ghost" onClick={() => { if (imageUrl) window.open(imageUrl, "_blank"); }}><ExternalLink /></Button>
                    </div>
                  </div>
                </div>
                ...
                <div className={clsx("p-4 rounded-xl border col-span-1 md:col-span-2", isDark ? "bg-black/20 border-zinc-800" : "bg-white/70 border-zinc-200")}>
                  <div className="text-sm font-semibold mb-2">Profile Summary</div>
                  ...
                  {profileJson ? (
                    ...
                    <div className="py-6 text-sm opacity-70">No profile JSON available. Gravatar will still serve an avatar at the image URL above. Try fetching after entering an email or a valid MD5 hash.</div>
                  )}
                </div>
              </div>
            ...
            <div className={clsx("p-4 border-t flex items-center justify-between", isDark ? "bg-black/60 border-zinc-800" : "bg-white/90 border-zinc-200")}>
              ...
              <div className="flex gap-2 items-center">
                <Button variant="outline" onClick={() => { navigator.clipboard.writeText(buildGravatarUrl(hashInput || DEFAULT_RESOURCE.exampleHash, size, rating, defaultImage)); showToast("success", "Endpoint copied"); }}><Copy /> Copy Endpoint</Button>
                <Button variant="outline" onClick={() => { setShowRaw((s) => !s); }}><List /> {showRaw ? "Hide JSON" : "Show JSON"}</Button>
                <Button variant="ghost" onClick={() => { setImageUrl(buildGravatarUrl(DEFAULT_RESOURCE.exampleHash, size, rating, defaultImage)); setHashInput(DEFAULT_RESOURCE.exampleHash); setEmailInput(""); fetchProfile(DEFAULT_RESOURCE.exampleHash); }}><ArchiveRestore /> Reset</Button>
              </div>
            </div>
          </Card>
        </section>
        ...
        <aside className={clsx("lg:col-span-4 rounded-2xl p-4 space-y-4 h-fit", isDark ? "bg-black/40 border border-zinc-800" : "bg-white/90 border border-zinc-200")}>
          <div>
            ...
            <div className="mt-3 space-y-2">
              ...
              <div className="flex gap-2 mt-2">
                <Button variant="outline" onClick={() => { navigator.clipboard.writeText(DEFAULT_RESOURCE.code); showToast("success", "Code snippet copied"); }}><Copy /> Copy Snippet</Button>
                <Button variant="outline" onClick={() => { navigator.clipboard.writeText(`${DEFAULT_RESOURCE.endpointBase}${hashInput || DEFAULT_RESOURCE.exampleHash}?s=${size}&r=${rating}&d=${defaultImage}`); showToast("success", "Endpoint copied"); }}><ExternalLink /> Open</Button>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="mt-3 space-y-2 max-h-64 overflow-auto no-scrollbar">
              {favorites.length === 0 ? (
                ...
                favorites.map((f) => (
                  <div key={f.id} className="flex items-center gap-2 p-2 rounded-md border">
                    ...
                    <div className="flex flex-col gap-1">
                      ...
                      <Button size="sm" variant="ghost" onClick={() => removeFavorite(f.id)}><X /></Button>
                    </div>
                  </div>
                ))
              )}
        ...
        <DialogContent className={clsx("max-w-3xl w-full p-0 rounded-2xl overflow-hidden", isDark ? "bg-black/90" : "bg-white")}>
          ...
          <DialogFooter className="flex justify-between items-center p-4 border-t">
            ...
            <div className="flex gap-2">
              ...
              <Button variant="outline" onClick={() => { if (imageUrl) window.open(imageUrl, "_blank"); }}><ExternalLink /></Button>
            </div>
          </DialogFooter>
        </DialogContent>
      </Dialog>
