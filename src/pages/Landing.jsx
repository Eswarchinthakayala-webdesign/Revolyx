...
export default function LandingPage() {
  ...
  // New state for loading/animation sequence
  ...
  return (
    <>
      {/* Loader Overlay with Animation */}
      {loading && (
        <motion.div
          key="loader-overlay"
          className="fixed inset-0 z-[100] grid-background flex items-center justify-center bg-white dark:bg-zinc-950"
          initial={{ opacity: 1 }}
          // Fade out the entire overlay container after the text animation is done
          animate={{ opacity: 0 }} 
      ...
      {/* Main Content (Only render if not loading) */}
      {!loading && (
        <div className="min-h-screen text-zinc-900 dark:text-zinc-100 transition-colors relative overflow-hidden">
          ...
          >
            ...
            {/* DESIGN IMPROVEMENT: Subtle Code Backdrop */}
            <motion.div
            ...
            >
              <Code2 className="w-[800px] h-[800px] text-zinc-300 dark:text-zinc-700/50 opacity-10 rotate-12" />
            </motion.div>


            {/* Content (z-10 ensures it's above the code backdrop) */}
          ...
          <motion.h1 initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="text-4xl md:text-7xl font-extrabold tracking-tight mb-6" > Build the Future with{" "} <span className="bg-gradient-to-r from-gray-900 to-gray-600 dark:from-white dark:to-gray-400 bg-clip-text text-transparent"> Revolyx </span> </motion.h1>
            ...
            >
              {/* DESIGN IMPROVEMENT: Primary CTA button updated */}
              <Button
                className="rounded-full h-10 cursor-pointer font-semibold shadow-xl bg-zinc-900 text-white dark:bg-zinc-100 dark:text-zinc-900 hover:bg-zinc-700 dark:hover:bg-zinc-300 transition-all transform hover:scale-[1.03]"
                onClick={() => setOpen(true)}
              >
              Start Exploring Now <ArrowRight className="w-4 h-4" />
              </Button>
              <Link to="/docs">
                <Button variant="outline" className="rounded-full h-10 cursor-pointer border-zinc-400 dark:border-zinc-600 font-semibold hover:bg-zinc-100 dark:hover:bg-zinc-800">
                  View Docs
                </Button>
              </Link>
